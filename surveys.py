class Question:
    ...
    def __init__(self, question, choices=None, allow_text=False):
        ...
        """IF NO CHOICES INPUT, DEFAULTS TO [YES, NO] LIST"""
        if not choices:
            choices = ["Yes", "No"]

        self.question = question #question being asked
        self.choices = choices #answer choices. see above for default
        self.allow_text = allow_text #allow for comment or not

    def __repr__(self):
        return f"<Question: question={self.question}, choices={self.choices}>"
